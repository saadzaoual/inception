...
project — to help scaffold the Dockerfiles and container entrypoint
scripts, to diagnose and repair a broken local git repository (object
files left with incorrect ownership after a container ran as root against
an NFS-mounted working directory), and to review the setup against this
subject's requirements and correct the gaps found (folder naming, the
data volume host path, explicit image names/tags, the Debian base version,
and this README's structure). Each change was reviewed against the subject
